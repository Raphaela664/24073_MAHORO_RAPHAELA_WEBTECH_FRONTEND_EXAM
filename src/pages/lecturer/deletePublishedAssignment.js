...
import deleteAssignmentAction from "../../redux/actions/lecturer/deleteAssignment";

const deleteAssignmentForm = ({ onClose, assignmentId, assignmentTitle }) => {
  const [isOpen, setIsOpen] = useState(true);
  const dispatch = useDispatch();
  const isLoading = useSelector((state) => state.deleteAssignment.isLoading);
  ...
  const handleSubmit = async () => {
    try {
      const response = await deleteAssignmentAction(dispatch, assignmentId);
      if (response) {
    ...
    <div className="custom-modal fixed inset-0 flex items-center justify-center">
      ...
      >
        ...
        <h3 data-testid="Sure" className="mb-3 text-center text-lg">
          ...
          <span className="font-bold text-center capitalize"> {assignmentTitle}?</span>
        </h3>
        <div className="flex">
          <button
            data-testid="Cancel"
